...
使用方法:
    docker compose exec django python manage.py shell < scripts/init_ai_consultation_data.py
    または
...
for data in consultation_types_data:
    ...
    if created:
        print(f"✓ 相談タイプを作成しました: {data['name']}")
    else:
        print(f"→ 相談タイプは既に存在します: {data['name']}")

# スーパーユーザーを取得（デフォルトスクリプトの作成者として使用）
superuser = User.objects.filter(is_superuser=True).first()
if not superuser:
    print("⚠️  警告: スーパーユーザーが見つかりません。デフォルトスクリプトの作成者を設定できません。")
    print("   スーパーユーザーを作成するには: python manage.py createsuperuser")
...
if consultation_types.get('税務相談'):
    ...
    if created:
        print("✓ 税務相談のデフォルトスクリプトを作成しました")
    else:
        print("→ 税務相談のデフォルトスクリプトは既に存在します")
...
if consultation_types.get('法律相談'):
    ...
    if created:
        print("✓ 法律相談のデフォルトスクリプトを作成しました")
    else:
        print("→ 法律相談のデフォルトスクリプトは既に存在します")
...
print("\n次のステップ:")
print("  1. サーバーを再起動: docker compose restart django")
print("  2. AI相談センターにアクセス: http://localhost:8000/ai-consultation/")
